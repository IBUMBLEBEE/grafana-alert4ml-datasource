...
/// * `rng` - Percentile range, default is (25.0, 75.0)
/// * `scale` - Scaling factor, default is 1.0
/// * `interpolation` - Interpolation method, default is "linear"
///
/// # Returns
///
/// Returns IQR value. Returns None if data is empty
pub fn iqr(
    data: &[f64],
    rng: Option<(f64, f64)>,
    scale: Option<f64>,
...
/// Dynamic threshold anomaly detection using IQR
///
/// # Arguments
///
/// * `data` - Input time series data
/// * `window_size` - Moving window size
/// * `threshold` - IQR multiplier threshold, default is 1.5
///
/// # Returns
///
/// Returns anomaly detection results, 1 indicates anomaly point, 0 indicates normal point
pub fn iqr_anomaly_detection(data: &[f64], window_size: usize, upper_percentile: Option<f64>, lower_percentile: Option<f64>, threshold: Option<f64>) -> Vec<f64> {
    ...
    // For each point, use the previous window_size points to calculate IQR
...
mod tests {
    ...
    fn test_iqr_anomaly_detection() {
        // Create test data containing anomaly points
        ...
        data[5] = 5.0; // Add an obvious anomaly point
        
        let window_size = 20; // Increase window size
        let threshold = 3.0; // Adjust threshold
        let anomalies = iqr_anomaly_detection(&data, window_size, Some(90.0), Some(10.0), Some(threshold));
        
        // Print number of anomaly points
        let anomaly_count = anomalies.iter().filter(|&&x| x == 1.0).count();
        println!("Total anomalies detected: {}", anomaly_count);
        
        // Print positions and values of anomaly points
