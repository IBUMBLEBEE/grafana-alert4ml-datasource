...
#[serde(rename_all = "camelCase")]
struct Schema {
    // Local patch (rsod/vendor): Grafana's FrameJSON marshals both with
    // `json:"...,omitempty"` — an empty frame name (prometheus, testdata,
    // ...) or missing refId omits the key entirely, which deserialization
    // rejected with `missing field 'name'`. Upstream 0.5.0 requires them.
    #[serde(default)]
    name: String,
    #[serde(default)]
    ref_id: String,
