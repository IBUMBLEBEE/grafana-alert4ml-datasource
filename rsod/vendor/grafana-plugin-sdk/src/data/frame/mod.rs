...
/// Frame type for wide-format time series, matching Go's
/// `data.FrameTypeTimeSeriesWide` (`FrameMeta.Type`).
pub const FRAME_TYPE_TIMESERIES_WIDE: &str = "timeseries-wide";
...
#[non_exhaustive]
pub struct Metadata {
    /// The frame type, e.g. [`FRAME_TYPE_TIMESERIES_WIDE`] (Grafana `FrameMeta.Type`).
