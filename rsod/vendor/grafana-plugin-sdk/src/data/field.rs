...
pub struct FieldConfig {
    ...
    /// Color configuration (Grafana `FieldConfig.Color`), e.g. a fixed color
    /// with `{"mode": "fixed", "fixedColor": "#808080"}`.
    #[serde(default)]
    pub color: Option<ColorConfig>,
}

/// Color configuration for a field, matching Grafana's `FieldColor` type
/// (`FieldConfig.Color` in the Go SDK).
#[skip_serializing_none]
#[derive(Clone, Debug, Default, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
#[non_exhaustive]
pub struct ColorConfig {
    /// The color mode: `"fixed"` to use [`ColorConfig::fixed_color`], or a
    /// palette mode such as `"palette-classic"`.
    #[serde(default)]
    pub mode: Option<String>,
    /// The color to use when [`ColorConfig::mode`] is `"fixed"`.
    #[serde(default)]
